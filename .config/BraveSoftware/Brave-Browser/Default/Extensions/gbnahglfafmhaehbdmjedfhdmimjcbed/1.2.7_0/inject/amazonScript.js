...
function checkForAddToCart(checkBoxId) {
  // console.log("works");
  // console.log(saleStarted,buttonNotClicked)
    ...
    return new Promise((resolve, reject) => {
      ...
      if (checked) {
        if (document.getElementById(checkBoxId)) {
          let addCart = document.getElementById(checkBoxId).parentElement.parentElement.parentElement;
          if (addCart) {
            let btn = addCart.querySelector("button");
            console.log("add", btn);
            if (btn && btn.innerText.includes("Cart")) {
        ...
        for (var key in res) {
          if (res.hasOwnProperty(key)) {
            ...
            if (res[key].URL == currentURL) {
              ...
              var Stime = new Date(saletime).getTime();
              var currentTime = new Date().getTime();
              if((currentTime < Stime - 120000) && (currentTime > Stime + 3000000)){
                ...
                Toast.fire({
                  title: productName,
                  html: "Autobuy Script is Running",
                });
              }
            }
          }
...
function timerStartScript(endtime) {
  ...
  var x = setInterval(function () {
    ...
    //   saleStarted = true;
    // }
    if((now > countDownDate - 120000) && (now < countDownDate + 3000000)){
      saleStarted = true;
      clearInterval(x);
    }else{
      saleStarted = false
